...
class SkillDownloadServiceTest {
    ...
    @Test
    void testDownloadVersion_AllowsAnonymousForTeamNamespaceNonPrivateSkill() throws Exception {
        ...
        SkillVersion version = new SkillVersion(2L, "1.0.0", "owner-1");
        setId(version, 10L);
        version.setStatus(SkillVersionStatus.PUBLISHED);
        ...
        when(visibilityChecker.canAccess(skill, null, Map.of())).thenReturn(true);
        when(skillVersionRepository.findBySkillIdAndVersion(2L, "1.0.0")).thenReturn(Optional.of(version));
        when(objectStorageService.exists("packages/2/10/bundle.zip")).thenReturn(false);
        when(skillFileRepository.findByVersionId(10L)).thenReturn(List.of(
                new SkillFile(10L, "SKILL.md", 4L, "text/markdown", "hash", "skills/2/10/SKILL.md")));
        when(objectStorageService.exists("skills/2/10/SKILL.md")).thenReturn(true);
        when(objectStorageService.getObject("skills/2/10/SKILL.md")).thenReturn(new ByteArrayInputStream("test".getBytes()));

        SkillDownloadService.DownloadResult result = service.downloadVersion("team-ai", "demo-skill", "1.0.0", null, Map.of());

        assertNotNull(result);
        assertEquals("Demo Skill-1.0.0.zip", result.filename());
        verify(skillRepository).incrementDownloadCount(2L);
        verify(skillVersionStatsRepository).incrementDownloadCount(10L, 2L);
        verify(eventPublisher).publishEvent(any(SkillDownloadedEvent.class));
    }
